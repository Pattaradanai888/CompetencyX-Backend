...
def seed_skill_assessment_catalog(apps, schema_editor):
    ...
    SkillAssessmentRoleGuidance = apps.get_model('assessments', 'SkillAssessmentRoleGuidance')
    Role = apps.get_model('roadmaps', 'Role')

    SkillAssessmentDimension.objects.bulk_create(
        [
            SkillAssessmentDimension(
                dimension_key=item['dimension_key'],
                label=item['label'],
                track=item['track'],
                low_score_action=item['low_score_action'],
                translations=item.get('translations', {}),
                display_order=item['display_order'],
            )
            for item in SKILL_ASSESSMENT_DIMENSIONS
        ]
    )
    SkillAssessmentQuestion.objects.bulk_create(
        [
            ...
            for item in SKILL_ASSESSMENT_QUESTIONS
        ]
    )
    roles_by_slug = {role.slug: role for role in Role.objects.filter(slug__in=[slug for slug in SKILL_ASSESSMENT_ROLE_GUIDANCE if slug])}
    SkillAssessmentRoleGuidance.objects.bulk_create(
        [
            ...
            if role_slug is None or role_slug in roles_by_slug
            for display_order, guidance in enumerate(guidance_items, start=1)
        ]
    )


def unseed_skill_assessment_catalog(apps, schema_editor):
    apps.get_model('assessments', 'SkillAssessmentRoleGuidance').objects.all().delete()
    apps.get_model('assessments', 'SkillAssessmentQuestion').objects.all().delete()
    apps.get_model('assessments', 'SkillAssessmentDimension').objects.all().delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(seed_skill_assessment_catalog, unseed_skill_assessment_catalog),
    ]
