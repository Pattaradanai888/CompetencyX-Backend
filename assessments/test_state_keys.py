...
``Survey2QuestionQValue`` and ``RecommendationQValue`` rows are keyed on these
...
from assessments.services.survey2_service import build_survey2_state_key
from recommendations.models import Recommendation
from roadmaps.models import Role


@pytest.mark.django_db
def test_survey2_state_key_format_is_stable():
    session = AssessmentSession.objects.create()

    assert build_survey2_state_key(session, {}) == 'none:unknown:in_progress:avg-2:progress-0'
    assert build_survey2_state_key(session, {'a': 5, 'b': 4}) == 'none:unknown:in_progress:avg-3:progress-0'


@pytest.mark.django_db
def test_survey2_state_key_uses_preferred_role_slug():
    ...
    key = build_survey2_state_key(session, {'a': 3, 'b': 3, 'c': 3})
