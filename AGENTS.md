...
`config/` holds Django settings and root URL wiring, with runtime settings in `config/settings/runtime.py` and test settings in `config/settings/test.py`. Feature apps live in `accounts/`, `api/`, `assessments/`, `roadmaps/`, and `recommendations/`; keep app logic close to its `models.py`, `serializers.py`, `views.py`, and `tests.py`. Seeded and upstream content lives under `data/content/` and `data/upstream/`. Docker assets are in `docker/`, and coverage output is written to `coverage/`.
