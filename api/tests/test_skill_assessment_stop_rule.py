...
The assessment ends once further answers stop changing the top five
suggestions -- never before twelve answered items, never past twenty.
Reaching the ceiling without the suggestions settling completes the
assessment and reports that the result is less certain (ADR-0003).
"""
...
class StopRuleFlowTestCase(AssessmentFlowTestCase):
    ...
    def answer_first_n(self, session_id, count: int, *, values: dict[int, int] | None = None, reverse_order=False) -> dict:
        """Answer one item at a time, saving between each, as the UI does."""
        values = values or {}
        answers: dict[str, int] = {}
        indexes = range(1, count + 1)
        if reverse_order:
            indexes = range(SET_COUNT, SET_COUNT - count, -1)
        for index in indexes:
            answers[self.key(index)] = values.get(index, 1)
            response = self.save(session_id, answers)
            self.assertEqual(response.status_code, status.HTTP_200_OK, answers)
        return answers
...
class StopRuleTests(StopRuleFlowTestCase):
    def test_the_assessment_does_not_end_before_twelve_answers_even_when_settled(self):
        ...
        # Uniform low ratings: the top five stops moving after the second
        # answer, but the floor of twelve keeps the assessment asking.
        answers = self.answer_first_n(session_id, 11)
    ...
    def test_the_assessment_ends_once_the_top_five_settle(self):
        session_id = self.create_session()
    ...
    def test_the_assessment_never_asks_more_than_twenty_items(self):
        session_id = self.create_session()
        # Answering from the back of the catalog keeps rearranging the top
        # five, so nothing settles -- the ceiling is what stops the asking.
        answers = self.answer_first_n(session_id, 20, reverse_order=True)
    ...
    def test_the_respondent_sees_how_many_questions_remain(self):
        session_id = self.create_session()
        answers = self.answer_first_n(session_id, 3)
    ...
    def test_an_earlier_answer_can_be_revised(self):
        session_id = self.create_session()
        answers = self.answer_first_n(session_id, 4)
